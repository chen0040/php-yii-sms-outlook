...
<div data-role="page" id="target-dialog">
	<div data-role="header" data-theme="<?php echo $user->header_data_theme(); ?>">
		...
		<h1><?php echo Yii::t('translation', 'Update Contact') ?></h1>
	...
	<form id="target-form" method="post" action="<?php echo Yii::app()->createUrl('contact/updateContact', array('url'=>$url, 'field_id'=>$field_id, 'id'=>$model->id)); ?>" data-ajax="false" enctype="multipart/form-data">
		
		<ul data-role="listview" data-inset="true">
			
			<li style="height:140px">
			<?php if(isset($model)): ?>
			<?php echo CHtml::image($model->getImagePathIfFileExists(), 'Contact', array('style'=>'padding:10px;width:120px;height:140px;')); ?> 
			<?php endif; ?>
			</li>
		...
		<input type="submit" value="<?php echo Yii::t('translation', 'Update') ?>" id="UpdateContact" onclick="return validateBeforeSend();" name="UpdateContact" data-inline="true" data-icon="check" data-theme="<?php echo Yii::app()->accountMgr->getOKButtonDataTheme(); ?>" />
		<input type="submit" value="<?php echo Yii::t('translation', 'Delete'); ?>" name="Delete" data-inline="true" id="Delete" data-icon="delete" onclick="return confirmDelete();" />
		<input type="submit" value="<?php echo Yii::t('translation', 'Cancel'); ?>" name="Cancel" data-inline="true" id="Cancel" data-icon="back" />
		
		<script type="text/javascript">
		function confirmDelete()
		{
			if(confirm("<?php echo Yii::t('translation', 'Do you want to delete the contract').'?'; ?>"))
			{
				return true;
			}
			else
	...
	<div data-role="footer" data-theme="<?php echo $user->data_theme(); ?>">
		<h4>&nbsp;</h4>
	</div><!-- /footer -->
</div>