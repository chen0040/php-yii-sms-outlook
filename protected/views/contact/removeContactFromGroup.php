...
<?php $groups=Group::model()->findAll(); ?>
	<ul data-role="listview" data-theme="c" data-dividertheme="<?php echo $data_divider_theme; ?>">
		...
		Please click any group below to have the contact <?php echo $contact->first_name.' '.$contact->last_name; ?> removed from the group
		</li>
		
		<?php foreach($groups as $group): ?>			
			<?php if($group->hasContact($contact)): ?>
			<li>
			<a href="<?php echo Yii::app()->createUrl('groupContact/removeContactFromGroup', array('url'=>$url, 'field_id'=>$field_id, 'contact_id'=>$contact->id, 'group_id'=>$group->id)); ?>">
